...
CITY_SPEED = 19.44  # city street speeds in m/s
GAP_CLOSURE_SPEED = -1  # relative velocity between you and lead car which activates braking profile change [m/s]
TAILGATE_DISTANCE = 17.5  # when below this distance between you and lead car, braking profile change is active [m]
TAILGATE_VREL = 0  # relative velocity when tailgating in which profile should change [m/s]
MIN_DISTANCE = 7  # brake profile change when below this distance between you and lead car in [m]


# Braking profile changes (makes the car brake harder because it wants to be farther from the lead car)
...
class LongitudinalMpc(object):
  ...
  def update(self, CS, lead, v_cruise_setpoint):
    ...
    if x_lead < MIN_DISTANCE or (x_lead < TAILGATE_DISTANCE and self.v_rel < TAILGATE_VREL):
      self.tailgating = 1
    else:
      self.tailgating = 0
